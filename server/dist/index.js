...
const artilleryService_1 = require("./artilleryService");
...
io.on('connection', (socket) => {
    ...
    socket.on('subscribe', (resultId) => {
        ...
        const currentResult = artilleryService_1.artilleryService.getTestResult(resultId);
        if (currentResult) {
...
const updateInterval = setInterval(() => {
    ...
    const runningTests = artilleryService_1.artilleryService.getAllTestResults().filter(result => result.status === 'running');
...
app.post('/api/endpoints', (req, res) => {
    try {
        ...
        storage_1.storage.saveEndpoints(endpoints);
...
app.delete('/api/endpoints/:id', (req, res) => {
    try {
        ...
        storage_1.storage.saveEndpoints(newEndpoints);
        ...
        storage_1.storage.saveTests(updatedTests);
...
app.get('/api/tests', (req, res) => {
    res.status(200).json(storage_1.storage.getTests());
});
...
app.post('/api/tests', (req, res) => {
    try {
        ...
        storage_1.storage.saveTests(tests);
...
app.delete('/api/tests/:id', (req, res) => {
    try {
        ...
        storage_1.storage.saveTests(newTests);
        res.status(200).json({ success: true });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/tests/run', (req, res) => {
    try {
        const { test, endpoints } = req.body;
        if (!test || !endpoints || !Array.isArray(endpoints)) {
            return res.status(400).json({ error: 'Invalid request body' });
        }
        const resultId = artilleryService_1.artilleryService.runTest(test, endpoints);
        // Configurar un intervalo para enviar actualizaciones del estado de la prueba a través de WebSocket
        const updateInterval = setInterval(() => {
            const result = artilleryService_1.artilleryService.getTestResult(resultId);
            if (result) {
                ...
                if (result.status === 'completed' || result.status === 'failed') {
                    ...
                    setTimeout(() => {
                        const finalResult = artilleryService_1.artilleryService.getTestResult(resultId);
                        if (finalResult) {
                            ...
                            storage_1.storage.saveResult(finalResult);
                        }
                        clearInterval(updateInterval);
...
app.get('/api/tests/results', (req, res) => {
    try {
        ...
        const memoryResults = artilleryService_1.artilleryService.getAllTestResults();
        const storedResults = storage_1.storage.getResults();
...
app.post('/api/tests/cancel/:resultId', (req, res) => {
    try {
        const { resultId } = req.params;
        const success = artilleryService_1.artilleryService.cancelTest(resultId);
        if (!success) {
...
app.get('/api/export', (req, res) => {
    try {
        const exportPath = path_1.default.join(process.cwd(), 'data', 'export.json');
        const success = storage_1.storage.exportAllData(exportPath);
        if (!success) {
...
app.post('/api/import', (req, res) => {
    try {
        ...
        const importPath = path_1.default.join(process.cwd(), 'data', 'import.json');
        const success = storage_1.storage.importData(importPath);
        if (!success) {
...
server.listen(PORT, () => {
    ...
    console.log(`Data storage directory: ${path_1.default.join(process.cwd(), 'data')}`);
});
