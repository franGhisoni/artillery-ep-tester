...
exports.artilleryService = void 0;
 ...
 */
function estimateMetricsBasedOnProgress(result, test, elapsedMs) {
    ...
    if (estimatedRequestsTotal > result.summary.requestsCompleted) {
        ...
        if (Object.keys(result.summary.codes).length === 0) {
            ...
            if (code200 > 0)
                result.summary.codes['200'] = code200;
            if (code404 > 0)
                result.summary.codes['404'] = code404;
            if (code500 > 0)
                result.summary.codes['500'] = code500;
            if (code403 > 0)
                result.summary.codes['403'] = code403;
        }
 ...
 */
function generateArtilleryConfig(test, endpoints) {
    // Filtrar solo los endpoints incluidos en la prueba
    const selectedEndpoints = endpoints.filter((endpoint) => test.endpoints.includes(endpoint.id));
...
exports.artilleryService = {
     ...
     */
    runTest: (test, endpoints) => {
        const resultId = (0, uuid_1.v4)();
        const configFilePath = exports.artilleryService.generateConfigFile(test, endpoints);
            ...
            if (currentResult) {
                ...
                try {
                    ...
                    if (Object.keys(currentCodes).length > 0) {
                        const newCodes = { ...currentResult.summary.codes };
                                ...
                                if (report.aggregate) {
                                    ...
                                    Object.entries(currentResult.summary.codes).forEach(([code, count]) => {
                                        ...
                                        if (codeNum >= 10 && codeNum < 100) {
                                            if (codeNum >= 10 && codeNum < 20)
                                                normalizedCode = 418; // 4xx - Error del cliente
                                            else if (codeNum >= 20 && codeNum < 30)
                                                normalizedCode = 200; // 2xx - Éxito
                                            else if (codeNum >= 30 && codeNum < 40)
                                                normalizedCode = 304; // 3xx - Redirección
                                            else if (codeNum >= 40 && codeNum < 50)
                                                normalizedCode = 404; // 4xx - Error del cliente
                                            else if (codeNum >= 50 && codeNum < 60)
                                                normalizedCode = 500; // 5xx - Error del servidor
                                        }
                                        ...
                                        if (!stdCodes.includes(normalizedCode) && normalizedCode >= 100 && normalizedCode < 600) {
                                            ...
                                            switch (firstDigit) {
                                                ...
                                                case 4:
                                                    normalizedCode = 400;
                                                    break; // Error del cliente
                                                case 5:
                                                    normalizedCode = 500;
                                                    break; // Error del servidor
                                            }
                                        }
                                        ...
                                        if (!isNaN(normalizedCode) && normalizedCode >= 100 && normalizedCode < 600) {
                                            ...
                                            normalizedCodes[codeKey] = (normalizedCodes[codeKey] || 0) + count;
                                        }
                                    });
 ...
 */
function extractMetricsFromRawOutput(result) {
    try {
        ...
        if (foundCreated || foundCompleted) {
            result.summary.scenarios.failed = Math.max(0, result.summary.scenarios.created - result.summary.scenarios.completed);
        }
        ...
        for (const pattern of latencyPatterns) {
            ...
            if (match) {
                ...
                if (min > 0)
                    result.summary.latency.min = min;
                if (max > 0)
                    result.summary.latency.max = max;
                if (median > 0)
                    result.summary.latency.median = median;
                if (p95 > 0)
                    result.summary.latency.p95 = p95;
                if (p99 > 0)
                    result.summary.latency.p99 = p99;
                ...
                for (const pattern of field.patterns) {
                    ...
                    if (match && match[1]) {
                        ...
                        if (!isNaN(value) && value > 0) {
                            result.summary.latency[field.name] = value;
